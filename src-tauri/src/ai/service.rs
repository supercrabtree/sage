use crate::{config::AppConfig, FrontendMessage, MistralMessage, AiResponse, KnowledgeContext};
...
impl AiService {
    ...
    pub async fn send_messages_with_knowledge(&self, messages: Vec<FrontendMessage>, knowledge_context: Vec<KnowledgeContext>) -> Result<AiResponse, String> {
        if messages.is_empty() {
            return Err("No messages provided".to_string());
        }
        
        // Create enhanced system prompt with knowledge context
        let mut system_content = ROLE_PROMPT.trim().to_string();
        
        if !knowledge_context.is_empty() {
            system_content.push_str("\n\nAdditional context: The user has the following knowledge areas:\n");
            ...
            system_content.push_str("\nWhen relevant to the conversation, you may reference this knowledge to provide more informed responses. Only make connections or analogies when they genuinely enhance understanding - avoid forced comparisons. Focus on being helpful and accurate rather than drawing parallels for their own sake.");
        }
